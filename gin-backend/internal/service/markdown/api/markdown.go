...
func SetRouteGroup(public, protected *gin.RouterGroup) {
	// 保护路由组: /api/v1/protected/markdown (全部基于当前登录用户)
	mdProtected := protected.Group("markdown")
	mdProtected.POST("/upload", handler.UploadHandler)     // POST /api/v1/protected/markdown/upload  上传文章
	...
	mdProtected.GET("/:markdownId", handler.DetailHandler) // GET  /api/v1/protected/markdown/:markdownId 文章详情
}
