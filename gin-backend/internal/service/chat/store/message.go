...
type MessageStore interface {
	...
	FetchOffline(ctx context.Context, toID string, limit int) ([]ormchat.Message, error)
	FetchGroupHistory(ctx context.Context, groupID string, limit int) ([]ormchat.Message, error)
...
func (s *GormMessageStore) FetchOffline(ctx context.Context, toID string, limit int) ([]ormchat.Message, error) {
	db, err := s.dbConn()
	if err != nil {
		return nil, err
	}
...
func (s *GormMessageStore) FetchGroupHistory(ctx context.Context, groupID string, limit int) ([]ormchat.Message, error) {
	db, err := s.dbConn()
	if err != nil {
		return nil, err
	}
