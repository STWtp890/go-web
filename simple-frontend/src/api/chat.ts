...
export const chatApi = {
  ...
  join: async (groupId: string) => (await apiRequest<{ groupId: string; memberId: string }>(`/api/v1/protected/chat/groups/${encodeURIComponent(groupId)}/join`, { method: 'POST', scope: 'user' })).data,
  ...
  ack: (deliveryIds: string[]) => apiRequest<void>('/api/v1/protected/chat/deliveries/ack', {
    method: 'POST',
    scope: 'user',
    body: { deliveryIds },
  }),
