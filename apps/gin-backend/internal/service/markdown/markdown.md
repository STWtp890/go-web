...
- 生命周期: `internal/app` 在启动装配时通过 `PostgreSQLManager.RegisterAndGet` 注册, cleanup 中 `Unregister` 释放
...
- **能力边界**: go-web 只负责 BM25 全文检索，不生成 Embedding、不存储 vector 字段、
  不提供向量/语义检索接口；相关能力由独立 RAG + RPC 微服务实现。数据库中由 `pg_search`
  上游自动安装的 pgvector 仅为扩展运行时依赖，不进入 Markdown 领域模型或 API。
...
| `PUT` | `/api/v1/protected/markdown/:markdownId` | 全量更新自己的文章 |
| `DELETE` | `/api/v1/protected/markdown/:markdownId` | 删除自己的文章 |
...
- 更新采用全量 `PUT` 语义，`title`、`content`、`visibility` 均为必填。
- 只有作者可以编辑或删除；其他登录用户访问返回 403，不存在或已删除文章返回 404。
- 更新会重新生成 `summary` 与 `search_text`，元信息和正文在同一事务内提交。
- 删除会软删除 `markdowns` 元信息，并在同一事务内物理删除对应的 `markdown_contents`。
- 更新或删除成功后会失效元信息与正文缓存；写接口仍由 Cookie 会话和 CSRF 中间件保护。
...
- `keyword` 去除首尾空白后必须为 1–100 个 Unicode 字符。
- 只搜索当前登录用户自己的文稿，匹配 `title + summary + content` 组成的 `search_text`。
- 前端将已提交关键词和页码写入 URL 查询参数，浏览器前进/后退可恢复相同结果页。
...
│   ├── detail.go
│   └── mutation.go        # 作者编辑/删除、事务与缓存失效
└── types/
    └── requests/          # 请求 DTO

GORM 模型统一迁移至 `internal/model/orm/markdown/` (包名 markdown, 引用时常用别名
markdownmodel; 含 Markdown / Content), 与 auth 的 `internal/model/orm/auth/` 对齐;
时间字段共用 `internal/model/orm/base.go` 的 `TimeFiled`。
```
